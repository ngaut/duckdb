...
bool JitRegionExecutor::ExecuteFallback(PipelineExecutor &executor, JitRegionKernel &kernel, DataChunk &input,
                                        DataChunk &result, idx_t initial_idx, idx_t input_count_before,
                                        OperatorResultType &operator_result, string decline_reason,
                                        string fallback_reason, int64_t decline_time_us) {
	auto trace_runtime = Settings::Get<JitTraceRuntimeSetting>(executor.context.client);
	if (trace_runtime) {
		JitManager::Get(executor.context.client)
		    .RecordRuntimeEvent(executor.context.client, kernel, JitCompileTarget::REGION, "declined",
		                        std::move(decline_reason), input_count_before, 0,
		                        decline_time_us, "fallback");
	}

	std::chrono::steady_clock::time_point fallback_start;
	if (trace_runtime) {
		fallback_start = std::chrono::steady_clock::now();
	}
	try {
		{
			JitSuppressionGuard fallback_guard(executor.context.client);
			operator_result = executor.ExecutePipelineReference(input, result, initial_idx);
		}
		if (trace_runtime) {
			JitManager::Get(executor.context.client)
			    .RecordRuntimeFallbackEvent(executor.context.client, kernel, JitCompileTarget::REGION, "executed",
			                                std::move(fallback_reason), input_count_before, result.size(),
			                                JitRegionElapsedMicros(fallback_start),
			                                JitOperatorResultTypeToString(operator_result));
		}
		return true;
	} catch (...) {
		if (trace_runtime) {
			auto fallback_error = std::current_exception();
			JitManager::Get(executor.context.client)
			    .RecordRuntimeFallbackEvent(executor.context.client, kernel, JitCompileTarget::REGION, "error",
			                                "region executor fallback threw: " +
			                                    JitRegionExceptionMessage(fallback_error),
			                                input_count_before, 0, JitRegionElapsedMicros(fallback_start), "error");
		}
		throw;
	}
}

bool JitRegionExecutor::ExecuteSinkFallback(PipelineExecutor &executor, JitRegionKernel &kernel, DataChunk &input,
                                            OperatorSinkInput &sink_input, idx_t input_count_before,
                                            SinkResultType &sink_result, string decline_reason,
                                            string fallback_reason, int64_t decline_time_us) {
	auto trace_runtime = Settings::Get<JitTraceRuntimeSetting>(executor.context.client);
	if (trace_runtime) {
		JitManager::Get(executor.context.client)
		    .RecordRuntimeEvent(executor.context.client, kernel, JitCompileTarget::REGION, "declined",
		                        std::move(decline_reason), input_count_before, 0, decline_time_us, "fallback");
	}
	...
	try {
		{
			JitSuppressionGuard fallback_guard(executor.context.client);
			sink_result = executor.pipeline.GetSink()->Sink(executor.context, input, sink_input);
		}
		if (trace_runtime) {
			JitManager::Get(executor.context.client)
			    .RecordRuntimeFallbackEvent(executor.context.client, kernel, JitCompileTarget::REGION, "executed",
			                                std::move(fallback_reason), input_count_before, 0,
			                                JitRegionElapsedMicros(fallback_start),
			                                JitSinkResultTypeToString(sink_result));
		}
		return true;
	} catch (...) {
		if (trace_runtime) {
			...
			JitManager::Get(executor.context.client)
			    .RecordRuntimeFallbackEvent(executor.context.client, kernel, JitCompileTarget::REGION, "error",
			                                "sink pipeline executor fallback threw: " +
			                                    JitRegionExceptionMessage(fallback_error),
			                                input_count_before, 0, JitRegionElapsedMicros(fallback_start), "error");
		}
		throw;
...
bool JitRegionExecutor::TrySink(PipelineExecutor &executor, DataChunk &input, OperatorSinkInput &sink_input,
	...
	for (auto &candidate_kernel : executor.jit_kernels) {
		...
		if (!JitRegionABIIsSinkPipeline(contract.abi) ||
		    candidate_kernel->TraceCandidateStartOperatorIndex() != operator_count ||
		    candidate_kernel->TraceCandidateNodeCount() != 1 ||
		    !candidate_kernel->CanExecuteSinkPipeline()) {
			continue;
		}
		kernel = *candidate_kernel;
		break;
	...
	auto input_count_before = input.size();
	auto trace_runtime = Settings::Get<JitTraceRuntimeSetting>(executor.context.client);
	std::chrono::steady_clock::time_point trace_start;
	bool trace_started = false;
	try {
		...
		auto jit_executed = kernel->TrySink(executor.context, input, sink_input, sink_result);
		auto elapsed_us = trace_runtime ? JitRegionElapsedMicros(trace_start) : 0;
		if (!jit_executed) {
			return ExecuteSinkFallback(
			    executor, *kernel, input, sink_input, input_count_before, sink_result,
			    "sink pipeline kernel declined runtime input; executor fallback used",
			    "sink pipeline executor fallback executed after JIT decline", elapsed_us);
		}
		if (input.size() != input_count_before) {
			throw InternalException("JIT sink pipeline kernel changed input cardinality from %llu to %llu",
			                        static_cast<unsigned long long>(input_count_before),
			                        static_cast<unsigned long long>(input.size()));
		}
		if (trace_runtime) {
			JitManager::Get(executor.context.client)
			    .RecordRuntimeEvent(executor.context.client, *kernel, JitCompileTarget::REGION, "executed",
			                        "sink pipeline kernel executed", input_count_before, 0, elapsed_us,
			                        JitSinkResultTypeToString(sink_result));
		}
		return true;
	} catch (...) {
		...
		if (trace_runtime) {
			...
			JitManager::Get(executor.context.client)
			    .RecordRuntimeEvent(executor.context.client, *kernel, JitCompileTarget::REGION, "error",
			                        "sink pipeline kernel threw: " + JitRegionExceptionMessage(jit_error),
			                        input_count_before, 0, elapsed_us, "error");
		}
		throw;
	}
}
...
bool JitRegionExecutor::TryExecuteSinkPipeline(PipelineExecutor &executor, JitRegionKernel &kernel, DataChunk &input,
                                               DataChunk &result, idx_t initial_idx,
                                               OperatorResultType &operator_result) {
	D_ASSERT(JitRegionABIIsSinkPipeline(kernel.TraceCandidateContract().abi));
	if (!kernel.CanExecuteSinkPipeline()) {
		return false;
	}
	if (!executor.pipeline.GetSink()) {
		return false;
	...
	auto candidate_end = kernel.TraceCandidateEndOperatorIndex();
	if (candidate_end != operator_count) {
		throw InternalException("JIT sink pipeline candidate end %llu does not match operator count %llu",
		                        static_cast<unsigned long long>(candidate_end),
		                        static_cast<unsigned long long>(operator_count));
	}
	if (!executor.in_process_operators.empty()) {
		if (Settings::Get<JitTraceRuntimeSetting>(executor.context.client)) {
			JitManager::Get(executor.context.client)
			    .RecordRuntimeEvent(executor.context.client, kernel, JitCompileTarget::REGION, "declined",
			                        "sink pipeline kernel skipped because executor has in-process operators; "
			                        "reference executor continues current resume state;initial_idx=" +
			                            std::to_string(initial_idx),
			                        input_count_before, 0, 0, "fallback");
		}
		return false;
	}
	...
	try {
		...
		OperatorSinkInput sink_input {*executor.pipeline.GetSink()->sink_state, *executor.local_sink_state,
		                              executor.interrupt_state};
		executor.StartOperator(*executor.pipeline.GetSink());
		auto jit_executed = kernel.TrySink(executor.context, input, sink_input, sink_result);
		executor.EndOperator(*executor.pipeline.GetSink(), nullptr);
		auto elapsed_us = trace_runtime ? JitRegionElapsedMicros(trace_start) : 0;
		if (!jit_executed) {
			return ExecuteFallback(
			    executor, kernel, input, result, initial_idx, input_count_before, operator_result,
			    "sink pipeline kernel declined runtime input; executor fallback used;initial_idx=" +
			        std::to_string(initial_idx),
			    "sink pipeline executor fallback executed after JIT decline;initial_idx=" +
			        std::to_string(initial_idx),
			    elapsed_us);
		}
		if (input.size() != input_count_before) {
		...
		switch (sink_result) {
		case SinkResultType::NEED_MORE_INPUT:
			operator_result = OperatorResultType::NEED_MORE_INPUT;
			break;
		case SinkResultType::BLOCKED:
			operator_result = OperatorResultType::BLOCKED;
			break;
		case SinkResultType::FINISHED:
			executor.FinishProcessing();
			operator_result = OperatorResultType::FINISHED;
			break;
		default:
			throw InternalException("Unsupported JIT sink pipeline result");
		}
		if (trace_runtime) {
			JitManager::Get(executor.context.client)
			    .RecordRuntimeEvent(executor.context.client, kernel, JitCompileTarget::REGION, "executed",
			                        "sink pipeline kernel executed;initial_idx=" + std::to_string(initial_idx),
			                        input_count_before, 0, elapsed_us, JitSinkResultTypeToString(sink_result));
		}
		return true;
	} catch (...) {
		auto jit_error = std::current_exception();
...
bool JitRegionExecutor::TryExecute(PipelineExecutor &executor, DataChunk &input, DataChunk &result, idx_t initial_idx,
                                   OperatorResultType &operator_result) {
	if (executor.context.client.IsJitSuppressed()) {
		return false;
	}
	for (auto &candidate_kernel : executor.jit_kernels) {
		...
		auto &contract = candidate_kernel->TraceCandidateContract();
		if (JitRegionABIIsSinkPipeline(contract.abi) &&
		    candidate_kernel->TraceCandidateStartOperatorIndex() == initial_idx &&
		    candidate_kernel->CanExecuteSinkPipeline()) {
			return TryExecuteSinkPipeline(executor, *candidate_kernel, input, result, initial_idx, operator_result);
		}
	}
	...
	auto candidate_end = kernel->TraceCandidateEndOperatorIndex();
	if (candidate_end > operator_count) {
		throw InternalException("JIT region candidate end %llu exceeds operator count %llu",
		                        static_cast<unsigned long long>(candidate_end),
		                        static_cast<unsigned long long>(operator_count));
	}
	vector<LogicalType> candidate_output_types;
	if (candidate_end < operator_count) {
		candidate_output_types = kernel->TraceCandidateOutputTypes();
		if (candidate_output_types.empty()) {
			throw InternalException("JIT region candidate ends before pipeline end without output types");
		}
	} else {
		candidate_output_types = result.GetTypes();
	}
	DataChunk reference_input;
	DataChunk *execution_input = &input;
	if (verify_jit) {
		reference_input.Initialize(BufferAllocator::Get(executor.context.client), input.GetTypes());
		input.Copy(reference_input);
		execution_input = &reference_input;
	}
	...
	try {
		...
		DataChunk candidate_result;
		DataChunk *jit_result = &result;
		if (candidate_end < operator_count) {
			candidate_result.Initialize(BufferAllocator::Get(executor.context.client), candidate_output_types);
			jit_result = &candidate_result;
		}
		jit_executed = kernel->TryExecute(input, *jit_result, initial_idx, operator_result);
		if (jit_executed) {
			auto elapsed_us = trace_runtime ? JitRegionElapsedMicros(trace_start) : 0;
			if (input.size() != input_count_before) {
				throw InternalException("JIT region kernel changed input cardinality from %llu to %llu",
				                        static_cast<unsigned long long>(input_count_before),
				                        static_cast<unsigned long long>(input.size()));
			}
			if (verify_jit) {
				DataChunk reference_result;
				reference_result.Initialize(BufferAllocator::Get(executor.context.client), candidate_output_types);
				JitSuppressionGuard reference_guard(executor.context.client);
				auto expected_result = ExecuteReferenceOperatorInterval(executor, *execution_input, reference_result,
				                                                        initial_idx, candidate_end);
				VerifyRegionResult(*jit_result, operator_result, reference_result, expected_result);
			}
			if (trace_runtime) {
				JitManager::Get(executor.context.client)
				    .RecordRuntimeEvent(executor.context.client, *kernel, JitCompileTarget::REGION, "executed",
				                        "region kernel executed;initial_idx=" + std::to_string(initial_idx),
				                        input_count_before, jit_result->size(), elapsed_us,
				                        JitOperatorResultTypeToString(operator_result),
				                        GeneratedBodyRuntimeMetrics(elapsed_us));
			}
			if (candidate_end < operator_count) {
				operator_result = executor.ExecutePipelineOperators(*jit_result, result, candidate_end,
				                                                    executor.intermediate_chunks,
				                                                    executor.intermediate_states,
				                                                    executor.in_process_operators, true);
			}
			return true;
		}
	} catch (...) {
		...
		if (trace_runtime) {
			JitManager::Get(executor.context.client)
			    .RecordRuntimeEvent(executor.context.client, *kernel, JitCompileTarget::REGION, "error",
			                        "region kernel threw: " + JitRegionExceptionMessage(jit_error),
			                        input_count_before, 0, JitRegionElapsedMicros(trace_start), "error");
		}
		if (verify_jit) {
			try {
				DataChunk reference_result;
				reference_result.Initialize(BufferAllocator::Get(executor.context.client), candidate_output_types);
				JitSuppressionGuard reference_guard(executor.context.client);
				ExecuteReferenceOperatorInterval(executor, *execution_input, reference_result, initial_idx, candidate_end);
			} catch (...) {
				std::rethrow_exception(jit_error);
			}
			throw InternalException(
			    "JIT region verification failed: JIT threw \"%s\" but reference region execution succeeded",
			    JitRegionExceptionMessage(jit_error));
		}
		std::rethrow_exception(jit_error);
	}
	...
	if (trace_runtime) {
		return ExecuteFallback(
		    executor, *kernel, input, result, initial_idx, input_count_before, operator_result,
		    "region kernel declined runtime input; executor fallback used;initial_idx=" + std::to_string(initial_idx),
		    "region executor fallback executed after JIT decline;initial_idx=" + std::to_string(initial_idx),
		    JitRegionElapsedMicros(trace_start));
	}
	return ExecuteFallback(
	    executor, *kernel, input, result, initial_idx, input_count_before, operator_result,
	    "region kernel declined runtime input; executor fallback used;initial_idx=" + std::to_string(initial_idx),
