...
static void AddExecutionRegionEvent(QueryProfileResult &row, const ExecutionRegionEvent &event, bool is_runtime) {
	...
	AddFields(row, {{"runner_cost_profile", Value::BOOLEAN(event.runner_cost.present)}});
	...
	AddFields(row,
	          {{"runner_cost_source_contract_output_cardinality_unknown",
	           ...
	           {"runner_cost_required_runtime_proofs", NullableText(RenderExecutionRegionJitRuntimeProofRequirements(
	                                                       event.runner_cost.required_runtime_proofs))}});
