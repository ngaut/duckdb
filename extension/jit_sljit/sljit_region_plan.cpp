...
static void ApplySljitSourceRoute(const SljitSourceRoutePlan &route, ExecutionRegionLoweringPlan &lowering_plan) {
	lowering_plan.SetUsesScanFilters(route.uses_scan_filters);
	lowering_plan.SetRequiresSourceContractInputLayout(route.requires_source_contract_input_layout);
}

static vector<bool> BuildSljitSourceNotNullForRoute(const ExecutionRegionSourceInfo &source,
                                                    const SljitRegionNodePlan &node_plan) {
	if (node_plan.source_route.requires_source_contract_input_layout) {
...
ExecutionRegionLoweringPlan BuildSljitRegionPlan(const ExecutionRegionIR &region_ir,
	...
	SljitSourceRoutePlan selected_source_route;
	for (idx_t node_idx = candidate.first_node; node_idx < candidate.EndNode(); node_idx++) {
		...
		if (node.kind == ExecutionRegionNodeKind::SOURCE) {
			...
			if (executable_source && node_plan.kind == ExecutionRegionLoweringKind::NATIVE) {
				selected_source_route.Merge(node_plan.source_route);
				...
				if (node.source && node.source->table_scan_contract.present) {
					...
					native_region.source_not_null = BuildSljitSourceNotNullForRoute(*node.source, node_plan);
				}
			} else {
	...
	if (cursor.CanFuse() && !native_region.ops.empty()) {
		...
		ApplySljitSourceRoute(selected_source_route, lowering_plan);
