...
private:
	...
	using TryBuildProjectionAggregateFunction = bool (SljitFullPipelineRecipeBuilder::*)(
	...
	struct SljitProjectionAggregateRecipeRegistryEntry {
		...
		const char *name;
		TryBuildProjectionAggregateFunction try_build;
	};
	...
	static const SljitProjectionAggregateRecipeRegistryEntry *ProjectionAggregateRecipeRegistry(idx_t &count) {
		static const SljitProjectionAggregateRecipeRegistryEntry registry[] = {
		    {SljitProjectionAggregateRecipeKind::SOURCE, "source_grouped_aggregate",
		     &SljitFullPipelineRecipeBuilder::TryBuildSourceProjectionAggregateRecipe},
		    {SljitProjectionAggregateRecipeKind::SINGLE_JOIN, "single_join_mark_filter",
		     &SljitFullPipelineRecipeBuilder::TryBuildSingleJoinMarkFilterRecipe},
		    {SljitProjectionAggregateRecipeKind::SINGLE_JOIN, "single_join_source_filter_projection",
		     &SljitFullPipelineRecipeBuilder::TryBuildSingleJoinSourceFilterProjectionRecipe},
		    {SljitProjectionAggregateRecipeKind::SINGLE_JOIN, "single_join_pre_projection",
		     &SljitFullPipelineRecipeBuilder::TryBuildSingleJoinPreProjectionRecipe},
		    {SljitProjectionAggregateRecipeKind::SINGLE_JOIN, "single_join_direct_projection",
		     &SljitFullPipelineRecipeBuilder::TryBuildSingleJoinDirectProjectionRecipe},
		    {SljitProjectionAggregateRecipeKind::SINGLE_JOIN, "single_join_projection_chain",
		     &SljitFullPipelineRecipeBuilder::TryBuildSingleJoinProjectionChainRecipe},
		    {SljitProjectionAggregateRecipeKind::TWO_JOIN, "two_join_mark_filter",
		     &SljitFullPipelineRecipeBuilder::TryBuildTwoJoinMarkFilterRecipe},
		    {SljitProjectionAggregateRecipeKind::TWO_JOIN, "two_join_filtered_projection_prefix",
		     &SljitFullPipelineRecipeBuilder::TryBuildTwoJoinSourceFilterProjectionRecipe},
		    {SljitProjectionAggregateRecipeKind::TWO_JOIN, "two_join_pre_projection",
		     &SljitFullPipelineRecipeBuilder::TryBuildTwoJoinPreProjectionRecipe},
		    {SljitProjectionAggregateRecipeKind::TWO_JOIN, "two_join_between_projection",
		     &SljitFullPipelineRecipeBuilder::TryBuildTwoJoinBetweenProjectionRecipe},
		    {SljitProjectionAggregateRecipeKind::TWO_JOIN, "two_join_direct_projection",
		     &SljitFullPipelineRecipeBuilder::TryBuildTwoJoinDirectProjectionRecipe},
		    {SljitProjectionAggregateRecipeKind::TWO_JOIN, "two_join_projection_chain",
		     &SljitFullPipelineRecipeBuilder::TryBuildTwoJoinProjectionChainRecipe}};
		count = sizeof(registry) / sizeof(registry[0]);
		return registry;
	}

	bool TryBuildSourceProjectionAggregateRecipe(SljitFullPipelineRecipe &recipe,
		...
		recipe = binding.MakeSourceProjectionGroupedAggregateRecipe(plan.shape);
		return true;
	}

	bool TryBuildSingleJoinMarkFilterRecipe(SljitFullPipelineRecipe &recipe,
	                                        const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
		if (!facts.HasMarkFilter() || !SljitProjectionAggregateCanUseSingleJoinMarkFilterBoundary(ops, plan)) {
			return false;
		}
	...
	bool TryBuildSingleJoinSourceFilterProjectionRecipe(SljitFullPipelineRecipe &recipe,
	                                                    const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
		if (!facts.HasSourceFilterProjection() ||
		    !SljitProjectionAggregateCanUseSingleJoinSourceFilterProjection(plan) ||
	...
	bool TryBuildSingleJoinPreProjectionRecipe(SljitFullPipelineRecipe &recipe,
	                                           const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
		if (!facts.HasPreJoinProjection() || !SljitProjectionAggregateCanUseSingleJoinPreJoinProjection(plan) ||
		    !SljitCanBindHashJoinProbeSelectionPrimitive(ops, facts.first_hash_join_idx) ||
		    !DirectJoinProjectionAggregateHasDedicatedBackend(shape)) {
			return false;
		}
	...
	bool TryBuildSingleJoinDirectProjectionRecipe(SljitFullPipelineRecipe &recipe,
	                                              const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
	...
	bool TryBuildSingleJoinProjectionChainRecipe(SljitFullPipelineRecipe &recipe,
	                                             const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
	...
	bool TryBuildTwoJoinMarkFilterRecipe(SljitFullPipelineRecipe &recipe,
	                                     const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
	...
	bool TryBuildTwoJoinSourceFilterProjectionRecipe(SljitFullPipelineRecipe &recipe,
	                                                 const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
	...
	bool TryBuildTwoJoinPreProjectionRecipe(SljitFullPipelineRecipe &recipe,
	                                        const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
	...
	bool TryBuildTwoJoinBetweenProjectionRecipe(SljitFullPipelineRecipe &recipe,
	                                            const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
	...
	bool TryBuildTwoJoinDirectProjectionRecipe(SljitFullPipelineRecipe &recipe,
	                                           const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
		if (facts.HasSourceFilterProjection() || facts.HasPreJoinProjection() || facts.HasBetweenProjection() ||
	...
	bool TryBuildTwoJoinProjectionChainRecipe(SljitFullPipelineRecipe &recipe,
	                                          const SljitProjectionAggregatePlanFacts &plan) const {
		auto &shape = plan.shape;
		auto &facts = plan.prefix;
