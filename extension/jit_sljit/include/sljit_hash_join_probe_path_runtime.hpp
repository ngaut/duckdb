...
static SljitPreparedRegularHashJoinProbeInput SljitPrepareRegularHashJoinProbeInput(
    ...
    bool allow_unchecked_int64_to_int32, bool rhs_keys_have_null, bool use_bloom_filter) {
	...
	const SljitRegularHashJoinProbeInputShape input_shape {
	    ...
	    source_validity_array != nullptr, !rhs_keys_have_null};
	...
	native_input.entries = layout.entries.words;
	native_input.bitmask = layout.entries.bitmask;
	native_input.pointer_mask = layout.entries.pointer_mask;
	native_input.layout_kind = table_layout_kind;
	native_input.rhs_keys_have_validity = rhs_keys_have_null;
	native_input.pointer_offset = layout.pointer_offset;
	native_input.aux_next_ptrs = layout.entries.aux_next_ptrs;
	native_input.bloom_filter_bits = use_bloom_filter ? layout.bloom_filter.words : nullptr;
	native_input.bloom_filter_bitmask = use_bloom_filter ? layout.bloom_filter.bitmask : 0;
