...
SljitExecuteNativeAggregateUpdate(ExecutionRegionRuntime &runtime, ExecutionOperatorRuntime &native_runtime,
	...
	if (op.aggregate_update.plan.use_distinct_count_pointer_update) {
		if (execute_sel != nullptr || count != input.size()) {
			throw InternalException("SLJIT distinct count pointer update requires a materialized input chunk");
		}
		if (!binding.aggregate_update.state) {
			throw InternalException("SLJIT distinct count pointer update sink state is missing");
		}
		auto aggregate_stage_start = SljitRegionStageStart(runtime);
		auto sink_result = binding.aggregate_update.state->Sink(input);
		RecordSljitRegionStageRuntimePath(runtime, op_idx, op.kind, "distinct_count_pointer_update",
		                                  aggregate_stage_start);
		RecordSljitRegionMaterializationBoundary(runtime, op.kind, "distinct_count_pointer_update", input.size());
		return sink_result;
	}
