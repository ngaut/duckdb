...
static bool SljitPrepareProjectedAggregateInput(
    ...
    SljitExecutableRegionOp &semantic_projection, SljitDataChunkBatch &selected_hash_join_input,
    const char *materialized_context, SljitPreparedProjectedAggregateInput &prepared) {
	...
	if (input.HasHashJoinSelection()) {
		if (!SljitTryPrepareSelectedHashJoinProjectionInput(runtime, scratch, ops, primitive.final_projection_idx,
		                                                    ...
		                                                    prepared.source_chunk, prepared.projection_op)) {
			return false;
		}
