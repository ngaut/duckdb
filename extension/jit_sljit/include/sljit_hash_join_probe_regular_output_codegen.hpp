...
static inline void
EmitRetryRegularHashJoinProbeSlot(struct sljit_compiler *compiler,
	...
	EmitRetryHashJoinProbeSlot(compiler, control.probe_loop, control.salt_mismatch,
	                           key_jumps.equality_key_mismatches, restore_probe_offset,
	                           SLJIT_REGULAR_HASH_JOIN_BITMASK_REG_AVAILABLE,
...
static inline unique_ptr<ExecutionRegionCodeHandle> FinishRegularHashJoinMarkBuildOnlyOutput(
    ...
    const SljitRegularHashJoinProbeControlFlow &control, const vector<struct sljit_jump *> &source_is_null,
    const SljitRegularHashJoinProbeKeyJumps &key_jumps, bool restore_probe_offset) {
	...
	EmitSkipHashJoinProbeRow(compiler, control.row_loop, control.empty_slot, source_is_null, advance_row);
...
FinishRegularHashJoinMarkProbeOutput(struct sljit_compiler *compiler,
...
static inline unique_ptr<ExecutionRegionCodeHandle> FinishRegularHashJoinNoChainOutput(
    struct sljit_compiler *compiler, SljitNativeRegularHashJoinProbeFunction &function, string &error,
    const SljitRegularHashJoinProbeControlFlow &control, const vector<struct sljit_jump *> &source_is_null,
    const SljitRegularHashJoinProbeKeyJumps &key_jumps, bool restore_probe_offset) {
	...
	EmitSkipHashJoinProbeRow(compiler, control.row_loop, control.empty_slot, source_is_null, nullptr,
	                         advance_after_predicate);
...
static inline unique_ptr<ExecutionRegionCodeHandle> FinishRegularHashJoinMatchedProbeOnlyOutput(
	...
	auto predicate_mismatch = sljit_emit_label(compiler);
	SetSljitJumpLabels(key_jumps.predicate_key_mismatches, predicate_mismatch);
	EmitLoadHashJoinNextPointer(compiler, SLJIT_R0, SLJIT_R2, SLJIT_R4, plan.pointer_offset, config,
	                            aux_next_ptrs_reg);
