...
static void ExecuteAllValidHashJoinChainMarkSelectionProbe(SljitNativeRegularHashJoinProbeInput &input,
...
static void ExecuteAllValidHashJoinChainMarkSelectionProbeForInput(SljitNativeRegularHashJoinProbeInput &input,
	...
	if (SljitHashJoinProbeLayoutUsesSalt(input.layout_kind)) {
		if (input.bloom_filter_bits) {
			ExecuteAllValidHashJoinChainMarkSelectionProbe<SELECTED, DICTIONARY_EMISSION, true, true>(
			    input, matcher, consumer);
		} else {
			ExecuteAllValidHashJoinChainMarkSelectionProbe<SELECTED, DICTIONARY_EMISSION, true, false>(
			    input, matcher, consumer);
		}
	} else {
		if (input.bloom_filter_bits) {
			ExecuteAllValidHashJoinChainMarkSelectionProbe<SELECTED, DICTIONARY_EMISSION, false, true>(
			    input, matcher, consumer);
		} else {
			ExecuteAllValidHashJoinChainMarkSelectionProbe<SELECTED, DICTIONARY_EMISSION, false, false>(
			    input, matcher, consumer);
		}
	}
}
...
template <bool SELECTED, class MATCHER, class CONSUMER>
static void ExecuteAllValidHashJoinChainMarkSelectionExistenceProbeForInput(
    SljitNativeRegularHashJoinProbeInput &input, MATCHER matcher, CONSUMER &consumer) {
	if (SljitHashJoinProbeLayoutUsesSalt(input.layout_kind)) {
		if (input.bloom_filter_bits) {
...
static void ExecuteAllValidHashJoinMarkSelectionProbeWithMatcherMode(
...
static void ExecuteAllValidHashJoinMarkSelectionProbeWithMatcher(
    const SljitNativeHashJoinProbePlan &plan, SljitNativeRegularHashJoinProbeInput &input, MATCHER matcher) {
...
template <bool SELECTED, bool CHAIN, SljitHashJoinMarkSelectionMode MODE, class MATCHER>
static void ExecuteAllValidHashJoinEqualityMarkSelectionProbeWithMatcher(
    const SljitNativeHashJoinProbePlan &plan, SljitNativeRegularHashJoinProbeInput &input, MATCHER matcher) {
	if (plan.mark_build_match) {
