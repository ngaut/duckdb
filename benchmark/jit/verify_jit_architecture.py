...
def verify_backend_contract_tests(root: Path) -> None:
    assert_required_text(
        ...
        [
            ...
            "string_substring",
            "function=substring",
            "JIT manager rejects non-compiled results with kernels",
            ...
            "JIT source-prefix ABI rejects false return after native source fetch",
            ...
            "SLJIT marks table-function full pipeline unsupported without native source protocol",
            "JIT full pipeline appends native result collector output",
            "result-collector-append",
            "generated native result collector append protocol",
            "native_result_collector_append_contract=ready",
            ...
            "JIT hash join probe natively drains non-equality duplicate chains",
...
def verify_region_executor_resume_contract(root: Path) -> None:
    ...
    assert_required_text(
        ...
        [
            ...
            "JitRegionExecutor::HasSourcePrefixKernel(*this)",
            "HasJitSourcePrefixKernelRequiringNativeSource(jit_kernels)",
    ...
    assert_no_text(
        ...
        [
            ...
            Path("test/sql/jit/test_jit_framework.test"),
        ...
        [
            "sink" + "_helper_",
            "operator" + "_helper_input_rows",
            "operator" + "_helper_output_rows",
            "operator" + "_helper_invocation",
            "operator" + "_helper_runtime_time_us",
            "source_boundary_input_rows",
            "source_boundary_output_rows",
            "source_boundary_invocation",
            "source_boundary_runtime_time_us",
            "DUCKDB_GETDATA_HELPER",
            "duckdb-getdata-helper",
            "GetData helper",
    ...
    assert_no_text(
        root,
        [Path("src/execution/jit_region_executor.cpp"), Path("src/include/duckdb/execution/jit/region_executor.hpp")],
        [
            "ExecuteFallback",
            "JitRegionExecutor::TryExecute(PipelineExecutor",
            "region kernel skipped because executor has in-process operators",
            removed_name("region executor fallback executed after JIT de", "cline"),
            removed_name("source-prefix kernel de", "clined after source boundary fetch"),
    ...
    assert_required_text(
	    ...
	    [
	        ...
	        "JitRegionSinkKind::RESULT_COLLECTOR_APPEND",
	        "generated native result collector append protocol",
    ...
    assert_required_text(
        ...
        [
            ...
            "ExecuteNativeResultCollectorAppend",
            "JitAppendNativeResultCollector",
            "SljitNativeRegionOpKind::RESULT_COLLECTOR_APPEND",
        ],
    )
    assert_no_text(
        root,
    ...
    assert_required_text(
        ...
        [
            ...
            "finalization",
            "Compiled region kernels are non-declinable.",
            "Runtime preconditions belong in the",
            "core records",
            "raises an error instead",
            "Full-pipeline eligibility is also an executor-entry contract",
            "RequiredPartitionInfo().AnyRequired()",
            "maximal `source_prefix` prefix",
            "must not manufacture split candidates",
            "Sink-owned suffixes are not planner products or runtime contracts",
...
def verify_native_operator_runtime_contract(root: Path) -> None:
    ...
    assert_required_text(
        ...
        [
            ...
            "hash-join-native-runtime-non-equality-output-mode-protocol-invariant",
    ...
    assert_no_text(
        root,
        [Path("src/include/duckdb/execution/jit/runtime.hpp"), Path("src/execution/jit_runtime.cpp")],
        [
            removed_name("SetRuntimeDe", "clineReason"),
            removed_name("ConsumeRuntimeDe", "clineReason"),
            removed_name("runtime_", "de", "cline_reason"),
        ],
    )
...
def verify_native_sink_update_contract(root: Path) -> None:
    ...
    assert_no_text(
        ...
        [
            ...
            "source_boundary_output_rows += result",
            "source_boundary_invocation_count++",
        ],
    )
    assert_no_text(
        root,
...
def verify_region_selection_contract(root: Path) -> None:
    ...
    assert_required_text(
        ...
        [
            ...
            "this whole region",
            "CompileRegion(input)",
            "status != JitCompileStatus::COMPILED && result.kernel",
            "returned kernel for non-compiled region status",
    ...
    assert_required_text(
        ...
        [
            ...
            "GetJitRegionInventoryOperatorBoundary",
            "inventory.operator_names.size() == inventory.operator_boundaries.size()",
            "JitRegionBoundaryKindToString(boundary)",
    ...
    assert_required_text(
        ...
        [
            ...
            "SljitNativeRegionOpKind::HASH_JOIN_BUILD",
            "CanEnterFullPipeline",
            "CanBindNativeOperators",
    ...
    assert_required_text(
        ...
        [
            ...
            "JitRegionABIIsSourcePrefix(candidate.contract.abi)",
            "result++",
        ],
    )
    assert_required_text(
        ...
        [
            ...
            "source-boundary or partial operator success",
            "candidate_scope",
            "planner output is intentionally narrow",
            "`source_prefix` for one maximal",
            "full_pipeline",
            "core-owned source-prefix runtime ABI",
...
def verify_region_execution_form_contract(root: Path) -> None:
    ...
    assert_required_text(
        ...
        "extension/jit_sljit/sljit_region_runtime.cpp",
        [
            "sljit-full-pipeline-missing-native-source-protocol",
    ...
    assert_required_text(
        ...
        [
            ...
            "source_prefix",
            "native-source-prefix has invalid execution form",
            "source-boundary prefix was compiled as executable JIT",
    ...
    assert_required_text(
        ...
        [
            ...
            "Node counts, boundary counts,",
            "`execution_mode` and `region_execution_form` are intentionally separate.",
            "`duckdb-source-boundary` is an explicit non-fused source boundary",
            "DuckDB owns source",
            ...
            "Core must not manufacture a source-boundary",
            "Full-pipeline selective scan regions have one source-execution candidate.",
            "Source-boundary",
            "candidates are diagnostic proof-gap evidence",
            ...
            "`execution=duckdb-source-boundary`",
...
def verify_stage_region_contract(root: Path) -> None:
    ...
    assert_required_text(
        ...
        [
            ...
            "bool JitRegionABIIsSourcePrefix",
    ...
    assert_required_text(
        ...
        [
            ...
            "JitRegionABIIsSourcePrefix(contract.abi)",
            "JitRegionABIIsFullPipeline(contract.abi)",
            ...
            "candidate.context_traits.operator_protocol_boundary_count",
    ...
    assert_required_text(
        ...
        [
            ...
            "not partial native success states",
            "unsupported variants must stay missing-protocol",
            "semantic boundary labels",
            "Inventory carries source execution kind and typed operator boundary labels",
            "must not rebuild source or operator ownership by parsing rendered pipeline text",
            "`source-boundary`, `source-executor-fallback`,",
            "`source-missing-protocol`, `operator-protocol-boundary`",
            "`source-native`,",
            "`operator-native`, and `sink-native`",
            "must not be counted as capability gaps",
            "Scope labels are observability-only",
            "`native_fusion_ready=true` only when",
            "no source-boundary ownership boundary",
...
def verify_core_lowering_boundary(root: Path) -> None:
    ...
    assert_required_text(
        ...
        [
            ...
            "chain-drain/resume protocol",
            "Typed non-equality match predicates are handled by the native",
            "MARK joins use the same",
            "native probe output mode",
...
def verify_backend_registration_boundary(root: Path) -> None:
    ...
    assert_required_text(
        ...
        [
            "source-fusion-gap:requires-native-source",
            "candidate-fusion-gap:source-boundary",
            "source-strategy=duckdb-source-boundary",
        ],
    )
    assert_no_text(
        root,
...
def verify_trace_contracts(root: Path) -> None:
    ...
    assert_required_text(
        ...
        [
            ...
            "timing, runtime rows, kernel reachability, and fallback",
