#!/usr/bin/env python3
#
# Run benchmark-runner timings for SLJIT generated-body micro shapes.
...
def collect_runs(args: argparse.Namespace) -> tuple[list, list]:
    ...
    for shape in SHAPES:
        ...
        by_shape[shape["shape"]] = (shape, by_policy)

    for shape_name, (shape, by_policy) in by_shape.items():
...
def write_manifest(args: argparse.Namespace, out_dir: Path) -> None:
    write_trace_manifest(
        ...
        generator="benchmark/micro/jit/micro_jit_benchmark.py",
        configuration={
...
def parse_args() -> argparse.Namespace:
    root = repo_root()
    parser = argparse.ArgumentParser(description="Run JIT microbenchmark admission speed proof")
    parser.add_argument(
        "--benchmark-runner",
        type=Path,
...
def main() -> int:
    ...
    if args.out_dir is None:
        args.out_dir = default_trace_output_directory("micro_benchmark")
